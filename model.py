...
class CNNModel(nn.Module):
    def __init__(self, input_channels=256, num_cls=15):
        super(CNNModel, self).__init__()

        self.conv1 = nn.Conv2d(input_channels, 8, kernel_size=1)
        self.pool = nn.MaxPool2d(kernel_size=1, stride=2)
        self.conv2 = nn.Conv2d(8, 8, kernel_size=1)
        self.conv3 = nn.Conv2d(8, 64, kernel_size=1)
        self.conv4 = nn.Conv2d(64, 64, kernel_size=1)
        self.conv5 = nn.Conv2d(64, 64, kernel_size=1)

        # Compute flattened size after conv and pool
...
class UNet2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        return UNet2DOutput(sample=sample, mid_block=mid_block)
